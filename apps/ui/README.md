...
The authenticated Inventory tab lists `rag.documents` state through `/api/documents/inventory`. It can filter by lifecycle status, exact document ID, object-key prefix, or one exact metadata key/value pair, and renders current/desired processing versions, latest lifecycle summary fields, timestamps, errors, source links, and curated metadata. Each inventory row can load durable lifecycle history through `/api/documents/history` without leaving the Inventory tab. Text inventory rows with source object keys can also queue reprocessing through `/api/documents/reprocess`; successful queue responses update the row's desired processing version and load version-specific lifecycle history.
