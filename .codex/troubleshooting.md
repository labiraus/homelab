...
- On May 14, 2026, the first Flux-managed `vllm` release looked deployed but could not schedule because a failed pre-Flux/manual `homelab-vllm` Helm release was still stored in the workload namespace and kept a stale `homelab-vllm-vllm` pod on the only GPU. Check both `helm -n flux-system status homelab-vllm` and `helm -n homelab status homelab-vllm`; remove only the stale workload-namespace release records/resources so Flux's `flux-system`-stored release remains authoritative.
- For the same vLLM rollout, startup probe restarts after about fifteen minutes were too aggressive for first-load model download and CUDA compilation on `helheim`. Keep the chart image pinned instead of using `latest`, and give the startup probe a long enough window before treating the model server as failed.
- If the vLLM pod stays alive but spends many minutes in engine initialization with `compilation_config`/CUDA graph setup in the logs, prefer `--enforce-eager` on the current RTX 2070 path. It trades some steady-state optimization for a much more operator-friendly startup profile.
- If the same GPU path remains stuck in model load after eager mode is enabled, downshift to a small unquantized starter model before promoting larger AWQ models. On May 14, 2026, `Qwen/Qwen2.5-7B-Instruct-AWQ` stayed alive but did not become ready in a useful window on `helheim`, so the default local smoke-test model moved to `Qwen/Qwen2.5-0.5B-Instruct` and the chart renders `--quantization` only when explicitly set.
- If a new `homelab` workload unexpectedly has only its app container while its chart creates STRICT `PeerAuthentication`, check the live namespace labels before debugging service code. The namespace should have `istio-injection=enabled`; without it, new pods miss sidecar injection and mTLS-only inbound paths such as `external -> assistant` can fail even when the Deployment is `Ready`.
...
- For the same symptom, confirm the MCP NetworkPolicy allows egress to namespace `data`, pods labeled `cnpg.io/cluster: app-db`, on TCP `5432`. A local chart fix is not live until the app chart is published and Flux reconciles it.
- Also confirm the `data/app-db` NetworkPolicy allows ingress from `app.kubernetes.io/instance: homelab-mcp`. If the MCP pod logs repeat `failed to receive message: read tcp ... connection reset by peer` while Postgres is healthy, a missing Postgres-side ingress allowlist entry is the likely cause. Check both `helm/data/postgres/values.yaml` and the Flux-facing `postgres` release values under `helm/bootstrap/flux-data/values.yaml`, because the HelmRelease override replaces the chart's base `allowedClients` list.
- If processor logs or Postgres logs show `column does not have dimensions` for `rag_embeddings_vector_cosine_idx`, check whether `rag.embeddings.vector` is still an unconstrained `vector` column from an earlier bootstrap. The processor schema and `sql/rag/schema.pgsql` should alter it to `vector(384)` before creating the HNSW cosine index.
