...
- Also confirm the `data/app-db` NetworkPolicy allows ingress from `app.kubernetes.io/instance: homelab-mcp`. If the MCP pod logs repeat `failed to receive message: read tcp ... connection reset by peer` while Postgres is healthy, a missing Postgres-side ingress allowlist entry is the likely cause.
