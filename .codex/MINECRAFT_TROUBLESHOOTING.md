...
- June 13, 2026 update: remove the temporary EvilCraft replacement for ATM11 `0.1.2`. The repo no longer sets `CF_EXCLUDE_MODS=evilcraft`, no longer removes `evilcraft-26.1.2-neoforge-1.2.96-*`, and no longer installs `evilcraft-26.1.2-neoforge-1.2.97.jar`; the server should use the EvilCraft jar bundled by the ATM11 `0.1.2` CurseForge server pack.
...
- June 10, 2026 finding: ATM11 `0.1.2` crashed while loading `kaisyn:village/birch_forest_romanian/streets/crossroad_04` with `java.lang.OutOfMemoryError: Java heap space`.
...
- June 17, 2026 finding: ATM11 `0.1.2` failed during FML startup because `fastsuite`, `cristellib`, and `gateways` require NeoForge `26.1.2.76` or newer while the repo still pinned `26.1.2.75`.
